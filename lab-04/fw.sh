...
script_body() {
    ...
    iptables -A FORWARD -i eth0 -s 192.58.100.0/24 -j DROP
    iptables -A INPUT -i eth0 -s 192.58.100.0/24 -j DROP
    #
    ...
    # <--- Dodajte pravila 
    #
    iptables -A INPUT -p tcp --dport 22 -s 10.0.1.21 -j ACCEPT
    iptables -A OUTPUT -p tcp --dport 22 -d 10.0.1.21 -j ACCEPT
    ...
    # <--- Dodajte pravila 
    #
    iptables -A OUTPUT -p tcp --dport 53 -o eth2 -j ACCEPT
    iptables -A OUTPUT -p udp --dport 53 -o eth2 -j ACCEPT
    ...
    # <--- Dodajte pravila 
    #
    iptables -A FORWARD -p tcp --dport 25 -d 198.51.100.10 -j ACCEPT
    ...
    # <--- Dodajte pravila 
    #
    iptables -A FORWARD -p tcp --dport 25 -s 192.51.100.10 -j ACCEPT
    iptables -A FORWARD -p tcp --dport 53 -s 192.51.100.10 -j ACCEPT
    ...
    # <--- Dodajte pravila 
    #
    iptables -A FORWARD -p tcp --dport 80 -d 198.51.100.11 -j ACCEPT
    iptables -A FORWARD -p tcp --dport 443 -d 198.51.100.11 -j ACCEPT

    echo "... SSH Access to mail-relay (on DMZ) is permitted only on port 1111"
    #
    # <--- Dodajte pravila
    #
    iptables -A FORWARD -p tcp --dport 1111 -d 198.51.100.10 -j ACCEPT
